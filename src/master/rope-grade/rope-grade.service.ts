    ...
    async findAll(page: number | 'all' = 1, limit: number = 10, ropeTypeName: string): Promise<{ data: any[], fetchedCount: number, totalCount: number }> {
        ...
        return {
            data: ropeGrade.map(ropeGrade => ({
                ...
                categoryName: `${ropeGrade.category.parentCategory?.name ? ropeGrade.category.parentCategory.name + '/' : ''}${ropeGrade.category.childCategory?.name ? ropeGrade.category.childCategory.name + '/' : ''}${ropeGrade.category.subCategory?.name ? ropeGrade.category.subCategory.name : ''}`,
